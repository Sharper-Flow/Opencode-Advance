...
func newDoctorCmd(state *commandState) *cobra.Command {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				if err == health.ErrUnknownScope {
					return newCLIError(2, "unknown scope %q; supported: mcp, plugins, skills, temporal, adv-assets, adv-plugin, adv-runtime, cross", scope)
				}
	...
	cmd.Flags().StringVar(&scope, "scope", "mcp", "Scope to check (supported: mcp, plugins, skills, temporal, adv-assets, adv-plugin, adv-runtime, cross)")
	...
	_ = cmd.RegisterFlagCompletionFunc("scope", func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return []string{"mcp", "plugins", "skills", "temporal", "adv-assets", "adv-plugin", "adv-runtime", "cross"}, cobra.ShellCompDirectiveNoFileComp
	})
	return cmd
}
