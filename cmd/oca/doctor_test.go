...
func TestDoctorCommand_PluginsScope(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(checkout, "index.js"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(tmp, "opencode", "opencode.json"), []byte(`{"plugin":["`+checkout+`/index.js"]}`), 0o644); err != nil {
		t.Fatal(err)
	}
